...
for i in range(n):
            f.write('python train.py --cell_type \'lstm\' --attention_type \'luong\' --depth %d --hidden_units %d --learning_rate %1.5f --dropout_rate %1.2f --batch_size %d --embedding_size 500 --num_encoder_symbols 30000 --num_decoder_symbols 30000 --save_freq %d --display_freq %d --model_dir \'model%d/\'\n\n' %(depth_sweep[i],hidden_units_sweep[i],learning_rate_sweep[i],dropout_rate_sweep[i],batch_size,save_freq,disp_freq,i))
