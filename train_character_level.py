...
from seq2seq_model_char_level import Seq2SeqModel


# Data loading parameters
tf.app.flags.DEFINE_string('source_vocabulary', 'data/all.shuf.anno.json', 'Path to source vocabulary')
tf.app.flags.DEFINE_string('target_vocabulary', 'data/char_json_data/all_char.shuf.code.json', 'Path to target vocabulary')
tf.app.flags.DEFINE_string('source_train_data', 'data/train_small.shuf.anno', 'Path to source training data')
tf.app.flags.DEFINE_string('target_train_data', 'data/train_small_char_level.shuf.code', 'Path to target training data')
tf.app.flags.DEFINE_string('source_valid_data', 'data/train_small.bpe.anno', 'Path to source validation data')
tf.app.flags.DEFINE_string('target_valid_data', 'data/train_small_char_level.bpe.code', 'Path to target validation data')

tf.app.flags.DEFINE_string('lr_decay', 'exponential_decay', 'the decay type')
tf.app.flags.DEFINE_string('loss_type', 'weighted', 'the loss type')
...
def create_model(session, FLAGS):
    ...
    config = OrderedDict(tf.app.flags.FLAGS.flag_values_dict())#OrderedDict(sorted(FLAGS.__flags.items()))
...
def train():
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=FLAGS.allow_soft_placement, 
        ...
        for epoch_idx in xrange(FLAGS.max_epochs):
            ...
            value = summary_pb2.Summary.Value(tag="avg_loss", simple_value=avg_loss)
