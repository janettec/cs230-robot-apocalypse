...
class Seq2SeqModel(object):
    ...
    def predict(self, sess, encoder_inputs, encoder_inputs_length):
        ...
        outputs = sess.run(output_feed, input_feed)
        #pdb.set_trace()
        #alignments = sess.run(self.decoder_last_state_decode[-1].alignment_history.stack(), input_feed)
        #pdb.set_trace()
        #self.plot_attention(alignments[:,0,:])
                                # GreedyDecoder: [batch_size, max_time_step]
        return outputs[0]       # BeamSearchDecoder: [batch_size, max_time_step, beam_width]

    #Cite: https://stackoverflow.com/questions/44613173/how-to-visualize-attention-weights-from-attentionwrapper
    def plot_attention(self,attention_map):
        attn_len = len(attention_map)
        f = plt.figure(figsize=(15, 10))
        ax = f.add_subplot(1, 1, 1)
        i = ax.imshow(attention_map, interpolation='nearest', cmap='Blues')
        cbaxes = f.add_axes([0.2, 0, 1.0, 0.03])
        cbar = f.colorbar(i, cax=cbaxes)
        #cbar.ax.set_xlabel('Alpha value (Probability output of the "softmax")', labelpad=2)
        ax.set_yticks(range(attn_len))
        ax.set_xlabel('Input Sequence')
        ax.set_ylabel('Output Sequence')
        ax.grid()
        plt.savefig('/home/jskarda/tf-seq2seq/output_full_attnVis/attn_plots/attn_2ex'+str(self.attn_ex)+'.png')
        
        plt.figure(figsize=(7,7))
        plt.imshow(attention_map)
        plt.colorbar()
        plt.savefig('/home/jskarda/tf-seq2seq/output_full_attnVis/attn_plots/simple_attn_2ex'+str(self.attn_ex)+'.png')
        self.attn_ex = self.attn_ex+1
